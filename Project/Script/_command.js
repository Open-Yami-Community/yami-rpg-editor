            ...
            const forceProcessAll = (commands, indent = 0) => {
                for (let i = 0; i < commands.length; i++) {
                    ...
                    try {
                        ...
                        } else if (command.buffer && command.buffer.length > 0) {
                            // 检查命令缓冲区中是否有折叠信息
                        ...
                        const parsed = Command.parse(command, commandList.varMap)
                        if (parsed && Array.isArray(parsed)) {
                            ...
                            for (let j = 0; j < parsed.length; j++) {
                                ...
                                if (content.break !== undefined) {
                                    ...
                                    this.outputDebugLine(emptyElement, linesArray)
                                    lineNumber++
                                ...
                                if (content.children !== undefined) {
                                    ...
                                    if (Array.isArray(content.children)) {
                                        forceProcessAll(content.children, indent + 1)
                                    }
                            ...
                            if (command.id === 'comment' && command.params?.comment) {
                                element.displayText = `注释: ${command.params.comment}`
                            } else {
                                element.displayText = Local.get('command.' + command.id) || command.id || '未知命令'
                            }
            ...
            try {
                if (element) {
                    ...
                    if (element.textContent) {
                        ...
                        displayText = textParts.join(' ').replace(/\s+/g, ' ').trim()
                    }
    ...
    const printCommands = (cmdList, indent = 0) => {
        ...
        for (let i = 0; i < cmdList.length; i++) {
            ...
            for (const key in cmd) {
                if (key !== 'id' && key !== 'commands' && key !== 'buffer' && key !== 'folded') {
                    const value = cmd[key]
        ...
        for (let i = 0; i < nodeLength; i++) {
            ...
            if (elementIndex < elementsCount) {
                ...
                if (element && commandList.expandedLineNumbers) {
                    const expandedLine = commandList.expandedLineNumbers.get(element)
