...
class CommandList extends HTMLElement {
    ...
    update() {
        ...
        if (!window._completeCommandData && this.data && this.data.length > 100) {
            window._completeCommandData = this.data
        }
        ...
        if (this.currentSearchTerm) {
            ...
            if (!savedTerm || typeof savedTerm !== 'string' || savedTerm.trim() === '') {
                this.currentSearchTerm = null
                this.searchMatches = null
                this.currentMatchIndex = -1
    ...
    updateOnResize(element) {
        if (element.contents !== null) {
            ...
            if (this.currentSearchTerm) {
                setTimeout(() => {
                    if (element.textContent && element.textContent.includes(this.currentSearchTerm)) {
                        // 检查是否已经高亮
                        if (!element.querySelector('mark')) {
                            // 简单的普通高亮，让refreshAllHighlights处理当前项
                            this.highlightTextInElement(element, this.currentSearchTerm, false)
                        }
                    }
                }, 10)
            }
    ...
    parseConditionToText(condition) {
        try {
            ...
            if (condition.variable) {
                if (condition.variable.type === 'actor' && condition.variable.key) {
                    // 尝试从varMap获取变量名
                    if (this.varMap && this.varMap[condition.variable.key]) {
                        variableText = this.varMap[condition.variable.key]
                    } else {
                        // 尝试构造可读名称
                        variableText = this.constructActorVariableName(condition.variable)
                    }
            ...
            if (condition.operand) {
                ...
                } else if (condition.operand.type === 'variable' && condition.operand.key) {
    ...
    collectCommandInfo(element, lineNumber, isFolded = false) {
        ...
        // 检查是否是选项元素（条件分支的选项）
        const isOptionElement = element.tagName === 'COMMAND-ITEM' && element.classList.contains('option')
        ...
        if (cmd) {
            ...
            if (cmd.id === 'script' && cmd.params && cmd.params.script) {
                // 分割脚本为多行
                const scriptLines = cmd.params.script.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n')
                ...
                for (const [existingLineNumber, existingInfo] of this.allCommands) {
                    if (existingInfo.command === cmd && existingLineNumber < lineNumber) {
                        scriptLineIndex++
                    }
                }
            ...
            } else if (element && element.contents && Array.isArray(element.contents)) {
            ...
            if (element && element.contents && Array.isArray(element.contents)) {
                for (const content of element.contents) {
                    if (content && content.text !== undefined) {
                        text += content.text
                    }
            ...
            if (cmd.id === 'if') {
                ...
                elements.forEach((elem, index) => {
                    ...
                    if (hasElse && elements.length === 2) {
                        ...
                        text = index === 0 ? (Local.get('command.if.else') || '否则') : (Local.get('command.if.end') || '结束')
                ...
                elements.forEach(elem => {
                    // 使用本地化文本
                    elem.info.text = Local.get('command.loop.end') || '重复以上内容'
                    elem.info.needsDelayedCollection = false
                })
                ...
                elements.forEach((elem, index) => {
                    ...
                    // 先尝试从 contents 中获取文本
                    if (elem.info.element.contents && Array.isArray(elem.info.element.contents)) {
    ...
    processTriggerCommands() {
        ...
        // 获取所有命令信息按行号排序
        const sortedCommands = Array.from(this.allCommands.entries()).sort((a, b) => a[0] - b[0])
        ...
        for (let i = 0; i < sortedCommands.length - 1; i++) {
            ...
            if (currentInfo.command) {
                ...
                if (cmd.id === 'showText' && cmd.params && cmd.params.content) {
                    ...
                    if (nextInfo.command === cmd ||
                        (nextInfo.element && currentInfo.element &&
                            nextInfo.element.dataItem === currentInfo.element.dataItem)) {
                ...
                if (cmd.id === 'showChoices' && cmd.params && cmd.params.choices) {
    ...
    processChoicesCommand(cmd, sortedCommands, cmdIndex) {
        ...
        if (cmd.buffer && Array.isArray(cmd.buffer)) {
            ...
            for (let i = 0; i < cmd.buffer.length && optionIndex < choices.length; i++) {
                ...
                } else if (bufferItem && bufferItem.tagName === 'COMMAND-ITEM') {
                }
            }
            ...
            for (let i = 0; i < cmd.buffer.length; i++) {
                ...
                if (bufferItem &&
                    ...
                    (bufferItem.dataItem === cmd || !bufferItem.dataItem || bufferItem.dataItem === null)) { // 更宽松的条件
    ...
    processIfCommand(cmd, sortedCommands, cmdIndex) {
        ...
        if (cmd.buffer && Array.isArray(cmd.buffer)) {
            ...
            for (let i = 0; i < cmd.buffer.length && branchIndex < branches.length; i++) {
                const bufferItem = cmd.buffer[i]
                        ...
                        if (actualBranchIndex < branches.length) {
                            ...
                            try {
                                ...
                                for (let i = 0; i < contents.length; i++) {
                                    ...
                                    if (content.text && i === actualBranchIndex + 1) { // +1因为第0个是header
                                        conditionText = content.text
                                        break
                                    }
                                }
                            } catch (error) {
                                conditionText = this.parseIfConditions(branch, actualBranchIndex, branches.length)
                            }
            ...
            for (let i = 0; i < cmd.buffer.length; i++) {
                ...
                if (bufferItem &&
                    ...
                    bufferItem.classList &&
                    bufferItem.classList.contains('footer') &&
                    (bufferItem.dataItem === cmd || !bufferItem.dataItem || bufferItem.dataItem === null)) {
    ...
    parseCondition(condition) {
        ...
        const operation = operationMap[condition.operation] || condition.operation
        ...
        if (condition.operand) {
            ...
            } else if (condition.operand.type === 'variable' && condition.operand.variable) {
    ...
    createItems(commands, indent, parent = null) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            // 检查是否需要高亮这一行
            if (this.highlightTargetLine !== undefined && expandedLine === this.highlightTargetLine) {
                setTimeout(() => {
                    // 如果有搜索词，直接高亮文本
                    if (this.currentSearchTerm) {
                        // 再等一下确保内容加载
                        setTimeout(() => {
                            if (buffer[0].textContent && buffer[0].textContent.includes(this.currentSearchTerm)) {
                                this.highlightTextInElement(buffer[0], this.currentSearchTerm)
                            }
                        }, 100)
                    } else {
            ...
            if (buffer[0].dataItem?.folded) {
                ...
                if (this.currentSearchTerm && buffer[0].textContent) {
                    setTimeout(() => {
                        if (buffer[0].textContent.includes(this.currentSearchTerm)) {
                            this.highlightTextInElement(buffer[0], this.currentSearchTerm)
                        }
                    }, 50)
                }
            ...
            for (const target of buffer) {
                if (target instanceof HTMLElement) {
                    if (!isFirst) {
                        ...
                        // 检查是否需要高亮这一行
                        if (this.highlightTargetLine !== undefined && extraLine === this.highlightTargetLine) {
                            setTimeout(() => {
                                // 如果有搜索词，直接高亮文本
                                if (this.currentSearchTerm) {
                                    // 再等一下确保内容加载
                                    setTimeout(() => {
                                        if (target.textContent && target.textContent.includes(this.currentSearchTerm)) {
                                            this.highlightTextInElement(target, this.currentSearchTerm)
                                        }
                                    }, 100)
                                } else {
                    ...
                    if (this.currentSearchTerm && target.textContent) {
                        setTimeout(() => {
                            if (target.textContent.includes(this.currentSearchTerm)) {
                                this.highlightTextInElement(target, this.currentSearchTerm)
                            }
                        }, 50)
                    }
        ...
        const blankElement = this.createBlankElement(commands, length, indent, parent)
        const blankLine = this.currentExpandedLine++
    ...
    createCommandBuffer(commands, index, indent, parent) {
        ...
        if (buffer === undefined) {
            ...
            // 检查是否需要高亮这个元素
            if (this.pendingHighlightLine !== null && this.pendingHighlightLine !== undefined) {
                ...
                if (elementLine === this.pendingHighlightLine) {
                    ...
                    setTimeout(() => {
                        ...
                        if (this.currentSearchTerm) {
                            // 再等一下确保内容加载
                            setTimeout(() => {
                                if (li.textContent && li.textContent.includes(this.currentSearchTerm)) {
                                    this.highlightTextInElement(li, this.currentSearchTerm)
                                }
                            }, 100)
                        } else {
    ...
    performNewSearch(searchTerm, term) {
        ...
        otherSources.forEach(sourceName => {
            if (window[sourceName]) {
                ...
                const size = Array.isArray(source) ? source.length : (source.size || 'unknown');

                // 如果是数组且不太大，检查是否有匹配项
                if (Array.isArray(source) && source.length < 100) {
                    let matches = 0;
                    source.forEach((item, index) => {
                        const text = item.text || item.content || item.toString();
        ...
        // 智能检测当前项
        const initialIndex = window.determineCurrentSearchItem ? determineCurrentSearchItem(allMatches) : -1
        ...
        if (foldedMatches.length > 0) {
            ...
            // 计算展开前可见的匹配项数量
            const originalVisibleCount = allMatches.filter(m => !m.folded).length

            // 展开后重新搜索以获取准确的可见匹配数
            setTimeout(() => {
                this.refreshSearchAfterExpand(searchTerm, originalVisibleCount, hasAutoExpanded)
            }, 500)
        } else {
            // 更新计数器，只计算可见的匹配项
            ...
            if (window.updateSearchCounter) {
                window.updateSearchCounter(this.keyboardSelectedIndex, visibleMatches.length, searchTerm, hasAutoExpanded, 0)
            }
        }
    ...
    jumpToNthMatch(searchWord, index) {
        ...
        if (window.updateSearchCounter) {
            window.updateSearchCounter(this.keyboardSelectedIndex, this.searchMatches.length, this.currentSearchTerm)
        }
    ...
    refreshSearch(searchTerm, previousIndex = -1) {
        ...
        if (previousIndex >= 0 && previousIndex < allMatches.length) {
            ...
            this.currentMatchIndex = -1
            // 位置失效时重新智能检测当前项
            const newIndex = window.determineCurrentSearchItem ? determineCurrentSearchItem(allMatches) : -1
        ...
        // 更新计数器（使用快捷键选择索引）
        if (window.updateSearchCounter) {
            window.updateSearchCounter(this.keyboardSelectedIndex, allMatches.length, searchTerm)
        }
    }
        ...
        if (hiddenFoldedCount > 0) {
            ...
            if (hiddenFoldedCount > 0) {
                setTimeout(() => {
                    ...
                    this.refreshSearchAfterExpand(searchTerm, this.searchMatches?.length || 0, true)
                }, 300)
            }
        }
    }
    ...
    highlightVisibleMatches() {
        ...
        if (this.searchMatches && this.keyboardSelectedIndex >= 0 && this.keyboardSelectedIndex < this.searchMatches.length) {
        ...
        for (const element of elements) {
            if (element.textContent && element.textContent.includes(searchTerm)) {
                ...
                for (const mark of existingMarks) {
                    if (mark.textContent && mark.textContent.includes(searchTerm)) {
                ...
                if (!hasValidHighlight) {
                    ...
                    let isCurrent = false
                    // 现在都使用0-based系统，直接比较
                    if (targetLineNumber !== null && elementLineNumber === targetLineNumber) {
                        isCurrent = true
                        foundTarget = true
                    } else if (targetElementText && elementText.includes(targetElementText) && !foundTarget) {
                        isCurrent = true
                        foundTarget = true
                    }
                    ...
                    // 重新应用高亮（带当前项状态）
                    this.highlightTextInElement(element, this.currentSearchTerm, isCurrent)
                }
            }
        }
    }
    ...
    highlightTextInElement(element, term, isCurrent = false) {
        ...
        nodesToReplace.forEach(item => {
            ...
            const highlightedHTML = text.replace(regex, `<mark class="${markClass}">$&</mark>`)
    ...
    updateCurrentMatchHighlight() {
        if (!this.currentSearchTerm || !this.searchMatches || this.keyboardSelectedIndex < 0) return
        ...
        for (const element of elements) {
            ...
            if (elementLine === currentMatch.lineNumber) {
                ...
                this.clearHighlightsInElement(element)
                // 重新高亮，标记为当前项
                this.highlightTextInElement(element, this.currentSearchTerm, true)
                break
            }
        }
    }
    ...
    clearAllHighlights() {
        ...
        const containers = document.querySelectorAll('command-list, command-item, .command-content, .event-content')
    ...
    forceRemoveAllHighlights() {
        try {
            ...
            const yellowElements = commandList.querySelectorAll('[style*="background-color"], [style*="background"]')
            yellowElements.forEach((el) => {
                const style = el.getAttribute('style') || ''

                // 更宽泛的检查条件
                if (style.includes('#ffff00') || style.includes('yellow') || style.includes('rgb(255, 255, 0)') ||
                    style.includes('background-color: #ffff00') || style.includes('background-color:#ffff00') ||
                    style.includes('background: #ffff00') || style.includes('background:#ffff00')) {
                    try {
                        // 彻底清除所有背景相关样式
            ...
            // 清除所有搜索相关的CSS类
            const searchElements = commandList.querySelectorAll('.search-match, .search-highlight')
            ...
            // 清除带有搜索标识的元素
            const searchHighlightElements = commandList.querySelectorAll('[data-search-highlight="true"]')
            ...
            allElements.forEach((el) => {
                try {
                    ...
                    if (el.style) {
                        ...
                        if (bgColor && (bgColor.includes('#ffff00') || bgColor.includes('yellow') || bgColor.includes('rgb(255, 255, 0)'))) {
                            el.style.backgroundColor = ''
                        }

                        if (bg && (bg.includes('#ffff00') || bg.includes('yellow') || bg.includes('rgb(255, 255, 0)'))) {
                            el.style.background = ''
                        }
                    }
                    ...
                    if (styleAttr && (styleAttr.includes('#ffff00') || styleAttr.includes('yellow'))) {
                        // 重新设置style属性，过滤掉黄色背景
                        const newStyle = styleAttr.replace(/background[^;]*[#ffff00|yellow][^;]*;?/gi, '')
                    ...
                    // 检查类名
                    if (el.classList && (el.classList.contains('search-match') || el.classList.contains('search-highlight'))) {
                        el.classList.remove('search-match', 'search-highlight')
                    }
            ...
            allElements.forEach((el) => {
                try {
                    ...
                    if (window.getComputedStyle && el.nodeType === 1) {
                        ...
                        if (bgColor && (bgColor.includes('255, 255, 0') || bgColor === 'yellow' || bgColor === '#ffff00')) {
                            // 强制覆盖
                    ...
                    if (el.className && typeof el.className === 'string') {
                        ...
                        classes.forEach(cls => {
                            if (cls.includes('highlight') || cls.includes('search') || cls.includes('mark')) {
                    ...
                    if (el.tagName === 'SPAN' && el.style.backgroundColor) {
                        const bgColor = el.style.backgroundColor
                        if (bgColor.includes('#ffff00') || bgColor.includes('255, 255, 0') || bgColor.includes('yellow')) {
                            const parent = el.parentNode
                            if (parent) {
                                const textNode = document.createTextNode(el.textContent)
                                parent.replaceChild(textNode, el)
        ...
        try {
            ...
            if (!this.currentSearchTerm || this.currentSearchTerm.trim() === '') {
                ...
                const allHighlights = [
                    ...
                    ...commandList.querySelectorAll('[data-search-highlight="true"]'),
    ...
    clearHighlightsInElement(element) {
        ...
        try {
            ...
            // 清理残留的高亮相关元素
            const spansToCheck = element.querySelectorAll('span[style*="background"], span[style*="color"]')
            spansToCheck.forEach(span => {
                // 检查是否是高亮相关的span
                const style = span.getAttribute('style') || ''
                if (style.includes('background-color') && (style.includes('#ffff00') || style.includes('yellow'))) {
                    try {
    ...
    scrollToTargetLine(lineIndex, enableScroll = false) {
        ...
        const scrollTop = Math.max(0, targetScrollTop - centerOffset + lineHeight / 2)
    ...
    findParentCommand(targetInfo) {
        ...
        for (const folded of foldedCommands) {
            ...
            if (this.isLineInFoldedCommand(targetLine, folded.startLine, folded.info)) {
                return folded.info
            }
        }
        ...
        if (targetInfo.element) {
            ...
            if (targetDataItem && targetDataList) {
                ...
                for (const [lineIndex, info] of this.allCommands) {
                    if (info.command && info.command.folded) {
                        // 检查这个命令是否包含目标
                        if (this.commandContainsItem(info.command, targetDataItem, targetDataList)) {
                            return info
                        }
                    }
                }
    ...
    isLineInFoldedCommand(targetLine, foldedStartLine, foldedInfo) {
        ...
        for (let line = foldedStartLine + 1; line < this.allCommands.size; line++) {
        ...
        if (command.params) {
            ...
            if (command.params.branches) {
                for (const branch of command.params.branches) {
                    if (branch.commands === targetList ||
                        (branch.commands && branch.commands.includes(targetItem))) {
                        return true
                    }
                }
            }
            ...
            // 检查 elseCommands (if)
            if (command.params.elseCommands === targetList ||
                (command.params.elseCommands && command.params.elseCommands.includes(targetItem))) {
                return true
            }

            // 检查 defaultCommands (switch)
            if (command.params.defaultCommands === targetList ||
                (command.params.defaultCommands && command.params.defaultCommands.includes(targetItem))) {
                return true
            }

            // 检查 commands (loop等)
            if (command.params.commands === targetList ||
                (command.params.commands && command.params.commands.includes(targetItem))) {
                return true
            }
        }
...
window.showAllCommands = function() {
    const commandList = document.querySelector('command-list')
    if (commandList && commandList.allCommands && commandList.allCommands.size > 0) {
...
window.startHighlightCleanup = function() {
    ...
    highlightCleanupInterval = setInterval(() => {
        try {
            ...
            if ((!searchBox || !searchBox.value) && (!commandList || !commandList.currentSearchTerm)) {
                const marks = document.querySelectorAll('mark')
                if (marks.length > 0) {
                    if (window.forceRemoveAllMarks) {
                        window.forceRemoveAllMarks()
...
window.startHighlightCleanup = function() {
    ...
    highlightCleanupInterval = setInterval(() => {
        try {
            ...
            // 检查搜索框是否为空
            const isSearchEmpty = !searchBox || !searchBox.value || searchBox.value.trim() === ''

            // 检查命令列表是否有活跃搜索
            const hasActiveSearch = commandList && commandList.currentSearchTerm && commandList.currentSearchTerm.trim() !== ''
...
setTimeout(() => {
    ...
    if (searchBox) {
        ...
        searchBox.addEventListener('input', (e) => {
            ...
            if (!originalValue || originalValue === '' || !trimmedValue || trimmedValue === '') {
                ...
                if (commandList) {
                    ...
                    setTimeout(() => {
                        ...
                        // 额外确保计数器被清除
                        const counter = document.getElementById('search-counter-display')
                        if (counter) {
        ...
        if (searchPrev) {
            searchPrev.addEventListener('click', () => {
                const commandList = document.querySelector('command-list')
                if (commandList && typeof commandList.navigateToPrevMatch === 'function' && commandList.currentSearchTerm) {
        ...
        if (searchNext) {
            searchNext.addEventListener('click', () => {
                const commandList = document.querySelector('command-list')
                if (commandList && typeof commandList.navigateToNextMatch === 'function' && commandList.currentSearchTerm) {
...
function addNavigationKeyboardShortcuts() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.ctrlKey && e.key === 'ArrowUp') {
            ...
            if (typeof commandList.navigateToPrevMatch === 'function' && commandList.currentSearchTerm) {
                commandList.navigateToPrevMatch(commandList.currentSearchTerm)
            }
        }
        // Ctrl + 下箭头 = 下一项
            ...
            if (typeof commandList.navigateToNextMatch === 'function' && commandList.currentSearchTerm) {
                commandList.navigateToNextMatch(commandList.currentSearchTerm)
            }
        }
        // F3 = 下一项 (常见的查找下一项快捷键)
...
// 更新搜索计数器（基于快捷键导航的选择）
window.updateSearchCounter = function(keyboardIndex, totalCount, searchTerm = '', hasAutoExpanded = false, expandedCount = 0) {
...
window.determineCurrentSearchItem = function(searchMatches) {
    ...
    searchMatches.forEach((match, index) => {
        ...
        if (!element && match.lineNumber) {
            ...
            if (commandList.allCommands && commandList.allCommands.has(match.lineNumber)) {
                const commandInfo = commandList.allCommands.get(match.lineNumber)
            ...
            if (!element) {
                ...
                for (const item of commandItems) {
                    if (item.dataIndex === match.lineNumber) { // 现在都使用0-based系统
                        element = item
            ...
            if (!element) {
                const commandItems = commandList.querySelectorAll('command-item')
                for (const item of commandItems) {
                    if (item.textContent && match.text &&
        ...
        if (element && element.getBoundingClientRect) {
            try {
                ...
                // 检查是否在视窗内
                if (elementRect.bottom > listRect.top && elementRect.top < listRect.bottom) {
                    visibleMatches++
...
function addScrollListener() {
    ...
    const handleScroll = () => {
        ...
        scrollTimeout = setTimeout(() => {
            ...
            // 高亮新出现的匹配元素
            if (commandList.currentSearchTerm && commandList.highlightVisibleMatches) {
